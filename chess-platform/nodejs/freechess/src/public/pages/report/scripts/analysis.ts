...
async function evaluate1(pgn : string) {
...
$("#status-message").css("padding", "10px 3px 10px 3px");
logAnalysisInfo("Fetching PGN data...");
...
// Fetch PGN data from the server running on port 3000
$("#status-message").css("padding", "10px 3px 10px 3px");
logAnalysisInfo("Fetching PGN data...");
let pgn ='';
let depth = parseInt($("#depth-slider").val()!.toString());
...
try {
    const pgnResponse = await fetch("http://localhost:3000/api/pgn", {
        method: "POST",
        headers: {
            "Content-Type": "application/json",
        },
